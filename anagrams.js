...
for (var i = 0; i < words.length; i++) {
	if (words[i].length == 12){
		wordsWith12.push(words[i]);
	}
}

// console.log(wordsWith12);
...
// (You shouldn't use an object to do this, it's a warm up.)
let thoseTwoLettersArr = [];
for (var i = 0; i < words.length; i++) {
	if(words[i].includes("j") || words[i].includes("c")){
		thoseTwoLettersArr.push(words[i]);
	}
}

// console.log(thoseTwoLettersArr);
